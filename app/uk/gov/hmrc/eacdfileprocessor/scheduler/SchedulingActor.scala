...
class SchedulingActor extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case message: ScheduledMessage[_] =>
      logger.info(s"Picked up scheduled job [${message.getClass.getSimpleName}] for execution")
...
object SchedulingActor {
  ...
  case class ProcessApprovedFileMessage(service: ProcessApprovedFileService)
    extends ScheduledMessage[Either[Unit, LockResponse]]

  case class DeEnrolmentWorkItemPullMessage(service: ScheduledService[Either[Unit, LockResponse]])
    extends ScheduledMessage[Either[Unit, LockResponse]]

  case class ExpiredFileDeletionMessage(service: ExpiredFileDeletionService)
    extends ScheduledMessage[Either[Unit, LockResponse]]

  case class FileStatusUpdateMessage(service: ScheduledService[Either[Unit, LockResponse]])
    extends ScheduledMessage[Either[Unit, LockResponse]]
