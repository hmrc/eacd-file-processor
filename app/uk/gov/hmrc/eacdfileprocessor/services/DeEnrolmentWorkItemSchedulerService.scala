...
class DeEnrolmentWorkItemSchedulerService @Inject()(
  ...
  private def callES1AndProcessResult(
    ...
    logger.debug(s"[callES1AndProcessResult] Calling ES1 for work item ${workItemId.toHexString} with enrolmentKey $enrolmentKey and actionType $actionType")
      ...
      espConnector.callES9(groupId, enrolmentKey).flatMap { response =>
        ...
        response.status match {
          ...
          case _ =>
            ...
            recordError(reference, recordDetail, errorMessage, lastProcessBatch).map(_ => ()) // <-- Added .map(_ => ()) here
        }
      }
