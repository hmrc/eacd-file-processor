...
class UploadProgressTracker @Inject()(repository: FileRepository,
  ...
  def registerUploadResult(fileReference: Reference, details: Details)(implicit hc: HeaderCarrier): Future[Unit] =
    for {
      fileStatus <- details match {
        case _: Details.UploadedFailed       => Future.successful(FAILED)
        case _: Details.UploadedSuccessfully => Future.successful(SCANNED)
      }
      _ <- repository.updateStatusAndDetails(fileReference, fileStatus, details).map {
        case Some(_) if fileStatus == SCANNED =>
          val uploadedDetails = details.asInstanceOf[UploadedSuccessfully]
          transferToObjectStore(
            downloadUrl   = uploadedDetails.downloadUrl,
