...
case class AuthRequest[A](request : Request[A],
                          ...
                          retrieval : Retrieval.Username) extends WrappedRequest(request)

object AuthRequest {
  def apply[A, R](authenticatedRequest: AuthenticatedRequest[A, Retrieval.Username]): AuthRequest[A] = {
    AuthRequest(authenticatedRequest.request, authenticatedRequest.headerCarrier, authenticatedRequest.authorizationToken,
