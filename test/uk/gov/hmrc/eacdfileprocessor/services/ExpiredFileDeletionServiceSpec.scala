...
import uk.gov.hmrc.eacdfileprocessor.models.FileStatus.{FAILED, INITIAL}
...
class ExpiredFileDeletionServiceSpec extends TestSupport with TestData with UnitSpec {
  ...
  "ExpiredFileDeletionService" should {
    ...
    "skip processing when lock is already held" in new Setup {
      ...
      verify(fileRepository, never()).findExpiredInitialFiles
    ...
    "delete expired initial records directly from mongo" in new Setup {
      ...
      when(fileRepository.findExpiredInitialFiles).thenReturn(Future.successful(Seq(expiredInitial)))
      when(fileRepository.findExpiredActiveFiles).thenReturn(Future.successful(Seq.empty))
      when(fileRepository.deleteByReference(expiredInitial.reference)).thenReturn(Future.successful(true))

      val result: Either[Unit, LockResponse] = await(service.invoke)

      result shouldBe Left(())
      verify(fileRepository).findExpiredInitialFiles
      verify(fileRepository).deleteByReference(expiredInitial.reference)
    ...
    "delete expired active files from object store and then mongo" in new Setup {
      ...
      when(fileRepository.findExpiredInitialFiles).thenReturn(Future.successful(Seq.empty))
